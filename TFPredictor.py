...
class MambaBlock(nn.Module):
    def __init__(self, args: ModelArgs, kfgn: KFGN):
        ...
        self.args = args
        self.kfgn = kfgn
        self.use_fused_selective_scan = selective_scan_fn is not None
    ...
    def graph_delta_projection(self, delta, d_in):
        temp_adj = self.kfgn.gc_list[-1].get_transformed_adjacency()
        graph_dim = min(temp_adj.size(0), d_in)
        total_delta = delta.sum(dim=-1, keepdim=True)
        delta_p = total_delta.expand(-1, -1, d_in).clone()
        graph_delta = delta[:, :, :graph_dim]
        non_graph_sum = delta[:, :, graph_dim:].sum(dim=-1, keepdim=True)
        delta_p[:, :, :graph_dim] = torch.matmul(graph_delta, temp_adj[:graph_dim, :graph_dim]) + non_graph_sum
        return delta_p

    def selective_scan_fused(self, u, delta_p, A, B, C, D):
        # mamba-ssm 的 fused selective_scan_fn 使用 [batch, dim, length] 布局。
        # 这里的 delta_p 已经经过 softplus 和图结构修正，因此 delta_softplus=False。
        y = selective_scan_fn(
            rearrange(u, 'b l d_in -> b d_in l').contiguous(),
            rearrange(delta_p, 'b l d_in -> b d_in l').contiguous(),
            A.contiguous(),
            rearrange(B, 'b l n -> b n l').contiguous(),
            rearrange(C, 'b l n -> b n l').contiguous(),
            D.contiguous(),
            z=None,
            delta_bias=None,
        ...
        return rearrange(y, 'b d_in l -> b l d_in')

    def selective_scan(self, u, delta, A, B, C, D):
        (b, l, d_in) = u.shape
        n = A.shape[1]
        delta_p = self.graph_delta_projection(delta, d_in)

        if self.use_fused_selective_scan and u.is_cuda:
            try:
                return self.selective_scan_fused(u, delta_p, A, B, C, D)
            except Exception:
                self.use_fused_selective_scan = False
